...
def newff(minmax, size, transf=None, trainf=train.train_gd):
    ...
    net = Net(minmax, net_co, layers, connect, trainf, error.SSE())
    return net


def newp(minmax, cn, transf=trans.HardLim(), trainf=train.train_delta):
    """
    ...
    net = Net(minmax, cn, [l],  [[-1], [0]], trainf, error.SSE())
    return net


def newc(minmax, cn, trainf=train.train_cwta):
    """
    ...
    net = Net(minmax, cn, [l], [[-1], [0]], trainf, error.SAE())

    return net


def newlvq(minmax, cn0, pc, trainf=train.train_lvq):
    """
    ...
    net = Net(minmax, cn1, [layer_inp, layer_out],
                            [[-1], [0], [1]], trainf, error.MSE())

    return net


def newelm(minmax, size, transf=None, trainf=train.train_gdx):
    """
    ...
    net = Net(minmax, net_co, layers, connect, trainf, error.MSE())
    return net


def newhop_old(target, transf=None, trainf=None):
    """
    ...
    net = Net([minmax] * ci, ci, [l], [[0], [0]], trainf, None)
    return net


def newhop(target, transf=None, max_init=10, delta=0, trainf=None):
    """
    Create a Hopfield recurrent network
    ...
    net = Net([minmax] * ci, ci, [l], [[-1], [0]], trainf, None)
    return net


def newhem(target, transf=None, max_iter=10, delta=0, trainf=None):
    """
    ...
    connect = [[-1], [0], [1]]
    net = Net(minmax, cn, layers, connect, trainf, None)
    return net
